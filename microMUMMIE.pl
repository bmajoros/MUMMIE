...
my $baseDir=`echo \$MUMMIE`;
chomp $baseDir;
die "set environment variable \$MUMMIE to the full path of the base MUMMIE package (which should contain several .hmm files)\n" if $baseDir eq "";

System("cp \$MUMMIE/*.{hmm,txt,schema} .");
...
System("rm -rf chunks") if -e "chunks";
System("rm -rf chunks2") if -e "chunks2";

System("mkdir chunks chunks2");

System("par2fastb.pl $twoBitFile $dataDir chunks $UTR_FILE $libraryName");

System("assemble-transcripts.pl chunks chunks2 ; rm -r chunks ; mv chunks2 chunks");

System("get-schema.pl chunks > nocons.schema");
System("cat nocons.schema > cons.schema ; echo 'phastcons : continuous' >> cons.schema");
...
if($WANT_CONSERVATION)
  { System("hmm-edit PARCLIP.hmm TRK targetscan"); 
...
unless($WANT_CONSERVATION)
  { System("hmm-edit PARCLIP.hmm DTRK phastcons") }
...
for(my $i=0 ; $i<$N ; ++$i) {
  ...
  System("parse -c 0.01 $DASH_G 5-45 -p -d PARCLIP.hmm chunks > chunk-preds.gff");
  ...
  System("get-chrom-coords.pl chunk-preds.gff predictions-var$var.gff");
  ...
  if($WANT_BULGE) {
    ...
    for(my $bulgeType=1 ; $bulgeType<=3 ; ++$bulgeType) {
      $type.="I";
      System("bulge.pl chunks seeds.fasta $bulgeType $var");
      System("get-chrom-coords.pl bulge-predictions-var$var.gff chunk-preds.gff");
