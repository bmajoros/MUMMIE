...
{
  String test="bill/fastb-WT/x.fastb";

  Regex r1("([^\/]+)\\.fastb");
  if(r1.search(test)) cout<<"r1 matched"<<endl;

  Regex r2("([^\/]+)\\\.fastb");
  if(r2.search(test)) cout<<"r2 matched"<<endl;

  Regex r3("([^\/]+)\\\\.fastb");
  if(r3.search(test)) cout<<"r3 matched"<<endl;

  Regex r4("([^\/]+)\\\\\.fastb");
  if(r4.search(test)) cout<<"r4 matched"<<endl;
...
"parse [options] <*.hmm> <data.fastb>\n\
   where: -p = use posterior Viterbi\n\
          -g 2,3-7,8,13 = emit gff, foreground is defined by state list\n\
	...
	if(foreground[q]) {
	  ...
	  if(inSite) { 
	    ...
	    if(begin+1<end && score>=cutoff)
	      emit(substrate,begin,end,score,tracks,seq,hmm);
